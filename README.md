...
**[ INSERIR AQUI A IMAGEM DO FLUXOGRAMA GERADA NO MERMAID/ERASER ]**
...
* **Licença de Uso:** Serviço gratuito (modelos *free tier*). Requer cadastro na plataforma para emissão de uma API Key, que deve ser configurada via variáveis de ambiente.
...
## 💻 Instruções de Instalação e Execução (Plug & Play)

O projeto foi configurado para exigir o mínimo de atrito do avaliador. As variáveis de ambiente já estão pré-configuradas no arquivo `app/.env` enviado junto ao projeto.
...
> Embora a plataforma OpenRouter atenda ao requisito do escopo por oferecer modelos *free tier*, os testes de estresse na infraestrutura comprovaram que a latência extrema dessas opções gratuitas inviabiliza o tempo de resposta em tempo real exigido por um sistema Andon.
> 
